...
class TestValidateInsertRequest(unittest.TestCase):
    ...
    def test_valid_rich_request_with_per_record_overrides(self):
        ...
        request = InsertRequest(
            records=[
                InsertRecord(data={"a": 1}, table="t2"),
                InsertRecord(data={"a": 2}, table="t2", upsert=Upsert(update_type=UpsertType.REPLACE, unique_columns=["a"])),
            ],
        )
        validate_insert_request(None, request)  # should not raise
    ...
    def test_table_in_both_places_raises_even_if_only_one_record_sets_it(self):
        request = InsertRequest(
            records=[InsertRecord(data={"a": 1}, table="t2"), InsertRecord(data={"a": 2})],
            table="t1",
        )
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)
    ...
    def test_record_level_upsert_forbidden_when_table_is_at_request_level(self):
        ...
        request = InsertRequest(
            records=[InsertRecord(data={"a": 1}, upsert=Upsert(unique_columns=["b"]))],
            table="t1",
    ...
    def test_request_level_upsert_forbidden_when_table_is_per_record(self):
        request = InsertRequest(
            records=[InsertRecord(data={"a": 1}, table="t1")],
            upsert=Upsert(unique_columns=["a"]),
        )
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)
    ...
    def test_too_many_records_raises(self):
        request = InsertRequest(records=[InsertRecord(data={"a": 1}) for _ in range(10001)], table="t1")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_exactly_max_records_is_valid(self):
        request = InsertRequest(records=[InsertRecord(data={"a": 1}) for _ in range(10000)], table="t1")
        validate_insert_request(None, request)  # should not raise
    ...
    def test_table_missing_from_one_record_raises(self):
        ...
        request = InsertRequest(records=[InsertRecord(data={"a": 1}, table="t1"), InsertRecord(data={"a": 2})])
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_empty_key_in_record_data_raises(self):
        request = InsertRequest(records=[InsertRecord(data={"": "value"})], table="t1")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_whitespace_only_key_in_record_data_raises(self):
        request = InsertRequest(records=[InsertRecord(data={"   ": "value"})], table="t1")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_none_value_in_record_data_raises(self):
        request = InsertRequest(records=[InsertRecord(data={"a": None})], table="t1")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_empty_string_value_in_record_data_raises(self):
        request = InsertRequest(records=[InsertRecord(data={"a": ""})], table="t1")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_whitespace_only_value_in_record_data_raises(self):
        request = InsertRequest(records=[InsertRecord(data={"a": "   "})], table="t1")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)
    ...
    def test_falsy_non_string_values_are_valid(self):
        ...
        request = InsertRequest(records=[InsertRecord(data={"a": 0, "b": False, "c": [], "d": {}})], table="t1")
        validate_insert_request(None, request)  # should not raise

    def test_request_level_table_alone_is_valid(self):
        request = InsertRequest(records=[InsertRecord(data={"a": 1}), InsertRecord(data={"a": 2})], table="t1")
        validate_insert_request(None, request)  # should not raise

    def test_per_record_table_alone_is_valid(self):
        request = InsertRequest(records=[InsertRecord(data={"a": 1}, table="t1"), InsertRecord(data={"a": 2}, table="t2")])
        validate_insert_request(None, request)  # should not raise
    ...
    def test_records_must_contain_insert_record_instances(self):
    ...
    def test_table_must_be_non_empty_string_if_provided(self):
        request = InsertRequest(records=[InsertRecord(data={"a": 1})], table="   ")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_table_is_optional_when_every_record_has_its_own(self):
        request = InsertRequest(records=[InsertRecord(data={"a": 1}, table="t2")])
        validate_insert_request(None, request)  # should not raise

    def test_record_data_must_be_a_non_empty_dict(self):
        request = InsertRequest(records=[InsertRecord(data={})], table="t1")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_record_data_must_be_a_dict(self):
        request = InsertRequest(records=[InsertRecord(data=["not", "a", "dict"])], table="t1")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_upsert_must_be_an_upsert_instance(self):
        request = InsertRequest(records=[InsertRecord(data={"a": 1})], table="t1", upsert="not-an-upsert")
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)

    def test_upsert_unique_columns_must_be_non_empty_list_of_strings(self):
        request = InsertRequest(records=[InsertRecord(data={"a": 1})], table="t1", upsert=Upsert(unique_columns=[]))
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)
    ...
    def test_per_record_upsert_is_also_validated(self):
        request = InsertRequest(
            records=[InsertRecord(data={"a": 1}, upsert=Upsert(unique_columns=[]))],
            table="t1",
        )
        with self.assertRaises(SkyflowError):
            validate_insert_request(None, request)
